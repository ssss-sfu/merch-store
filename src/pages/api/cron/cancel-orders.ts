...
export default async function handler(
  ...
  // Verify if request is a Vercel Cron Job or has a valid API key
  const isVercelCron = req.headers["x-vercel-cron"] === "1";
  const headerApiKey = req.headers["x-api-key"];
  const queryApiKey = req.query.apiKey as string | undefined;

  // Check both header or query for API key
  const hasValidApiKey =
    headerApiKey === process.env.CRON_API_KEY ||
    queryApiKey === process.env.CRON_API_KEY;
